...
* Read in the gene expression matrix of raw read counts 
...
cts <- as.matrix(read.table("/Users/OwenW/Desktop/all_counts.txt", 
...
We also need to read in the sample annotation that we downloaded from the SRA, which contains sample labels, experimental labels, and sequencing run information, etc. 
```{r}
# read in the file from the SRA that has sample/experimental labels 
sra_res <- read.csv("/Users/OwenW/Desktop/sra_result.csv", row.names=1)
head(sra_res)
...
sra_run <- read.csv("/Users/OwenW/Desktop/SraRunInfo.csv", row.names=1)
head(sra_run)
...
Now that we have the sample metadata loaded, we need to clean up some variables to get the information we need for downstream analysis. 
```{r}
...
The design formula let's DESeq2 know your experimental design and what groups you want to compare in the differential expression analysis. It requires a variable that is present in colData to be specified, and will be used for the statistical analysis in DESeq2. This varuiable should be a (`factor`) class variable, with the reference group for your comparison set as the first level of the variable. 
...
Although we generally try to pool samples together at similar concentrations in a sequencing run, some samples will end up being sequenced more than others, leading to slight differences in how many reads are produced for that sample, and therefore sequencing depth and size. Furthermore, if samples are sequenced on separate runs, their sequencing depths may be very different. If we don't account for this variation in sequencing depth, we might conclude some genes are expressed at greater levels in a samplen that has simply been sequenced to a higher depth.  

![Library size example](/Users/OwenW/Desktop/library_size.png)
...
![Library composition example](/Users/OwenW/Desktop/library_composition.png)
...
7. Divide the count for each gene in each sample by the scaling factor calculated for that sample. 
...
Once we have calculated the size factors, it can be helpful to look at their distribution to get a feel for how they vary and how much normalization between the sample is required. 
```{r}
sizeFactors(dds)
...
To perform mathematical procedures such as PCA, it is better to work with a transformed version of the counts, rather than the original untransformed values. DESeq2 actually utilizes its own transformation procedure, called the regularized logatrithm (rlog) implemented in the (`rlog()`) function. The rlog is similar in principle to a standard log transformation of the data, but is able to more appropriately transform the counts for gene with low expression values. 

DESeq2 is also capable of implementing the variance stabilising transformation (VST) for count data, and is generally suggested for larger datasets due to increase speed. 

For this analysis, we will use the rlog, which produces values on the log2 scale and are also normalized for library size during the rlog procedure. Lets perform the rlog transformation on our data. 
```{r}
rld <- rlog(dds, blind = FALSE)
head(assay(rld))
```

We can illustrate the benefit of using the rlog over standard log transformation (+ a pseudo-count for genes with 0 counts) by comparing the transformed values for two samples against each other. 
```{r}
par(mfrow=c(1,2))
...
Looks like 500 is approximately the number of genes at which the variance starts to spike upwards, so this is a good number of variable features to use. Lets restrict the dataset to these genes for purposes of the PCA. Now lets extract and visualize the variance explained by each PC to determine which are most informative. 
```{r}
...
Looking at the plot, we can see some clear clustering by treatment group for some of the samples. For example, **untreated** samples appear to have much lower PC1 values than all the **Dex** samples, suggesting there some of the largest variability in gene expression differences between samples in this dataset explains differences between **untreated** and **Dex** treated samples, and we should expect the most substantial differential expression to be found for the **untreated** vs **Dex** analysis. 

The **Alb** treated samples, and the co-treated samples **(Alb_Dex)** do not seem to consistently cluster along PC1 and PC2. One explanation for this could be that treatment with **Alb** or co-tx with **Alb + Dex** do not lead to consistent gene expression changes across study subjects. This is often the case with *in vivo* data sets, rather than those collected in *in vitro* systems such as cell culture, and may mean that a greater number of replicates is required to accurately assess the impact of such treatment of gene expression, as there are usually a greater number of uncontrollable variables in *in vivo* datasets (e.g. genotype). 

Another explanation could be there is another variable that explains a substantial amount of variability in the dataset, which is unrelated to sample groupings, such as a batch effect, that is driving the clustering observed in the PCA plot. For example, if the two Alb tx samples with high values for PC1 and PC2 were known to have been processed in a different batch to the Alb tx sample with lower values for these PCs, we could infer that the re is a batch effect that is driving the differences between these samples, rather than an interesting biological factor. Since no batch information was decribed or provided for this dataset, this may not be the case for these data, however they still as a useful example for exploring this concept. 

It is interesting that 4 samples, 1 from each tx group, have much higher PC2 values than all other samples. This is a pattern we might expect to see if these 4 samples were processed in a separate batch, and the gene expression variation captured by PC2 is related to the batch effect, rather than our biological factor of interest (tx group). For purposes of this example, lets create a fake variable for sample batch, and include this on the PCA plot. 
...
![Hierarchical clustering example](/Users/OwenW/Desktop/heatmaps.png)
...
There does indeed seem to be relatively good clustering between untreated and Dex samples, suggesting there are unique gene expression programs defining the Dex samples from the untreated. However, one of these samples in the Dex group seems to be clustered further away from the other Dex samples. Why could this be, perhaps it is the sample that clustered away from the other samples on the PCA. We can add an annotation bar for the fake batch effect we created earlier to this plot to help us figure this out. 
...
It seems that this is in fact the case, and these 2 samples are also outliers based on the hierachical clustering, which would support the presence of a batch effect if these data were infact collected in multiple batches. 

We may also wish to cluster based on the pairwise correlations between gene expression samples, as this is again another complimenatry aproach to help us understand how the samples gene expression programs are rleated to each other. 

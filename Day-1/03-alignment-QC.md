...
We then just need to make sure the `xxx.markduplicates_metrics.txt` file in included a subdirectory when we run `MultiQC`, and it will be included in the report. Lets also just have a quick look at it now. 
